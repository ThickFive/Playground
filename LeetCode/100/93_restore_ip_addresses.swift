/*	FILEPATH = "./LeetCode/100/93_restore_ip_addresses.swift"
...
class Solution {
    func restoreIpAddresses(_ s: String) -> [String] {
        ...
        func backtarck(_ path: [Int], _ length: Int) {
            ...
            var path = path
            for i in 1..<4 {
                if i + length > chars.count {
                    break
                }
                if i >= 2 {   //  前置0, 大于255
                    if Int(String(chars[length]))! == 0 {
                        break
                    }
                }
                if i == 3 {
                    if Int(String(chars[length]) + String(chars[length+1]) + String(chars[length+2]))! > 255 {
                        break
                    }
                }
